"""
Convert Reviews JSONL to Parquet Files
========================================
This script converts Electronics.jsonl (22 GB) to 10 optimized Parquet files.
Only reviews that exist in the 5-core whitelist are included.

Input:
- data/raw/Electronics.jsonl (~15-20M lines)
- data/raw/Electronics_pureid_5core.csv (whitelist)

Output:
- data/processed/reviews_Electronics/ (10 parquet files, ~8 GB total)

Expected: ~15,473,536 reviews filtered
Time: ~15-20 minutes
"""
...
print("=" * 100)
print("CONVERT REVIEWS JSONL TO PARQUET")
print("=" * 100)
print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")

# Configuration
INPUT_JSONL = '../data/raw/Electronics.jsonl'
WHITELIST_CSV = '../data/raw/Electronics_pureid_5core.csv'
...
# Step 2: Load whitelist from 5-core CSV
print("=" * 100)
print("STEP 2: Loading 5-Core Whitelist")
print("=" * 100)

whitelist = set()
print(f"📂 Reading: {WHITELIST_CSV}")

try:
    with open(WHITELIST_CSV, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            whitelist.add((row['user_id'], row['parent_asin']))
    
    print(f"✅ Loaded {len(whitelist):,} unique (user_id, parent_asin) pairs")
    print(f"   Sample pairs: {list(whitelist)[:3]}\n")
except FileNotFoundError:
    print(f"❌ ERROR: Whitelist file not found: {WHITELIST_CSV}")
    exit(1)
...
print("   Fields: rating, title, text, asin, parent_asin, user_id, timestamp, helpful_vote, verified_purchase\n")
...
for i in range(1, NUM_FILES + 1):
    ...
    # Writers will be created when first batch is ready
...
print(f"   Filtering by whitelist...")
    ...
    with open(INPUT_JSONL, 'r', encoding='utf-8') as f:
        for line_num, line in enumerate(f, 1):
            ...
            try:
                ...
                if (user_id, parent_asin) in whitelist:
            ...
            if line_num % PROGRESS_INTERVAL == 0:
                print(f"   Processed {line_num:,} lines... (Filtered: {stats['reviews_filtered']:,} reviews)")
    
    print(f"\n✅ Finished reading JSONL file")
    print(f"   Total lines: {stats['lines_processed']:,}")
    print(f"   Filtered reviews: {stats['reviews_filtered']:,}")
...
for i in range(1, NUM_FILES + 1):
    ...
    if os.path.exists(file_path):
        ...
        # Read parquet to get record count
...
metadata = {
    ...
    'whitelist': 'Electronics_pureid_5core.csv',
...
print(f"✅ Filtered {stats['reviews_filtered']:,} reviews")
...
# Save summary to log
summary_path = '../logs/convert_reviews_summary.txt'
with open(summary_path, 'w') as f:
    f.write(f"Review JSONL to Parquet Conversion Summary\n")
    f.write(f"==========================================\n")
    f.write(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
    f.write(f"Lines processed: {stats['lines_processed']:,}\n")
    f.write(f"Reviews filtered: {stats['reviews_filtered']:,}\n")
